locals {
  all_controls = {
    ...
    AmiChanges = {
      pattern     = "{ ($.eventName = RegisterImage) || ($.eventName = CreateImage) }"
      description = "A CloudWatch Alarm that triggers if and AMi is created."
    }

    AMIModification = {
      pattern     = "{ ($.eventName = DeregisterImage) || ($.eventName = CopyImage) || ($.eventName = ModifyImageAttribute) }"
      description = "A CloudWatch Alarm that triggers if an AMI is modified"
    }
    ...
    AmiChanges = {
      ...
      description = "A CloudWatch Alarm that triggers if changes to EC2 instances are made."
    }
    ...
    LargeEC2Changes = {
      ...
      description = "A CloudWatch Alarm that triggers if changes to Large EC2 instances are made."
    }

    S3CreationDeletion = {
      pattern     = "{ ($.eventSource = s3.amazonaws.com) && (($.eventName = DeleteBucket) || ($.eventName = CreateBucket)) }"
      description = "A CloudWatch Alarm that triggers if S3 bucket is created or Deleted."
    }
